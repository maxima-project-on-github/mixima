...
;(defun dispwx (COUNT h) 
;  (format t "<mth><lbl>Out[~a]:= </lbl><n>~a</n></mth>"  COUNT h)
;    )
...
(setf (get '|BigFloat| 'formatter) 'bigfloatformatter)

;;; sample definition of new formatter...  9/30/94
(setf (get '|Increment| 'formatter) 'postformatter)
(setf (get '|Increment| 'stringprec) 150) 
(setf (get '|Increment| 'stringform) "++ ")

;; probably could make use of more vertical/horizontal concatenators
;; have to keep track of precedences, though.

(setf (get '|Factorial|'formatter) 'postformatter)
(setf (get '|Factorial| 'stringprec) 152) ;guess  2/96RJF
(setf (get '|Factorial| 'stringform) "!")

(setf (get '|Factorial2| 'formatter) 'postformatter) 
(setf (get '|Factorial2| 'stringprec) 152) ;guess  2/96RJF
(setf (get '|Factorial2| 'stringform) "!!")

(setf (get '|PreIncrement| 'formatter) 'preformatter)
(setf (get '|PreIncrement| 'stringprec) 150) ;guess  9/30/94 RJF
(setf (get '|PreIncrement| 'stringform) "++")

;; rjf 2/96
(defun postformatter(x)			;single post operator
  (let ((op (car x))
	(st nil)
	(args (cdr x))
	(width 0)
...
(defun preformatter(x)   ;single pre operator
  (let ((op (car x))
	...
	(MakeHForm (+ width (atomwidth st))
		   (+ maxtoph maxbotmh) hoffset voffset op
		   (prec op) 'left
		   (list (get op 'stringform) ret_format ))))
...
(defun BuildFormat (x)
  ;; dispatches on Head of object, regardless of atom/cons/etc.
  (let ((type (|Head| x))  ;; could be fixnum, or Plus, or ..
	 formatfun)
    ;; dispatch on the Head of the form if it is an atom
    (cond ((and (atom type)(setq formatfun(get type 'formatter)))
	   (funcall formatfun x))
	  ;; if it is an atom but no special formatter program defined
	  ;; then use the default atom-head formatter.
	  ((atom type) (deflt-buildformat x))
	  ;; some types are strange like (Derivative 1) or (Inverse F)
	  ;; use another default formatter
	  (t (deflt-buildformat-op x)))))



;; set up number formatters
(defun numberformatter(x)   (if (minusp x) (minusformatter (|Abs| x)) x))

(mapc #'(lambda(x)(setf (get x 'formatter) #'numberformatter))
      '(single-float double-float fixnum bignum ratio)) ;not complex



(setf (get 'symbol 'formatter) #'(lambda(x) (if x x 'False))) ;; nil is False

(setf (get 'rat 'formatter) #'(lambda(x) (BuildFormat(outof-rat x))))
(setf (get 'complex 'formatter)
      #'(lambda(x)(complexnumformatter x))) ;fixed 12/19/95 RJF
(setf (get 'bigfloat 'formatter)
      #'(lambda(x)
	  (BuildFormat (format nil "~s" x))))
...
;;added 11/18/94 RJF
(defun alternativesformatter(x)(genformatter x 'Alternatives))
(setf (get 'Alternatives 'stringsym) " | ")
(setf (get 'Alternatives 'stringsymlen) 3)
(setf (get 'Alternatives 'stringprec) 140) ;; guess
(setf (get 'Alternatives 'formatter) 'alternativesformatter)
...
(defun realformatter (x)
	; (format t "a:~a:a" (|Plus| 0.0 (cadr x) (caddr x)))
